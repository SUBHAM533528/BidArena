      ...
      <nav className="flex-1 px-3 py-4 overflow-y-auto no-scrollbar space-y-0.5">
        {NAV.map(l => (
          <NavLink key={l.to} to={l.to} end={l.to === "/admin"} onClick={onClose}
            ...
            <span className="w-5 text-center shrink-0"><i className={`${l.icon} text-[14px]`} /></span>
            <span>{l.label}</span>
          </NavLink>
        ))}
      </nav>
      <div className="px-3 py-4 border-t dark:border-ink-800 border-ink-200 shrink-0 space-y-0.5">
        ...
        <NavLink to="/admin/change-password" onClick={onClose}
          className={({ isActive }) => `nav-link ${isActive ? "active" : ""}`}>
          <span className="w-5 text-center"><i className="fa-solid fa-lock text-[14px]" /></span>
          <span>Change Password</span>
        </NavLink>
        <button
          onClick={async () => { await logout(); navigate("/admin/login"); }}
          className="nav-link w-full text-left text-flame-500 hover:text-flame-400 hover:bg-flame-500/10">
          <span className="w-5 text-center"><i className="fa-solid fa-right-from-bracket text-[14px]" /></span>
          <span>Log out</span>
        </button>
      </div>
    </aside>
