    ...
    <div className="relative min-h-screen bg-[#0a0d0a] text-ink-100 font-body overflow-x-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8 h-16 flex items-center justify-between gap-4">
          <Link to="/" className="flex items-center gap-3 shrink-0 group">
            ...
            <span className="font-editorial text-2xl font-bold tracking-wide text-gold-400">
              BidArena<span className="font-normal text-ink-400 text-base"> Auctions</span>
            </span>
          </Link>
      ...
      <section className="relative min-h-[92vh] flex items-end overflow-hidden">
        ...
        <div className="absolute inset-0 bg-cover bg-center scale-105"
          style={{ backgroundImage: "url('https://images.unsplash.com/photo-1531415077968-de08abad10ab?auto=format&fit=crop&w=1920&q=80')" }} />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-8 pt-32 pb-16 md:pb-24 w-full">
          ...
          <div className="flex flex-wrap items-center gap-4 mb-8">
            <Link to="/owner/register"
              className="inline-flex items-center gap-2 px-6 py-3.5 bg-gold-500 hover:bg-gold-400 text-[#1a1206] font-bold text-sm rounded-lg transition shadow-lg shadow-gold-500/25 hover:-translate-y-0.5">
              Enter the Auction <i className="fa-solid fa-arrow-right text-xs" />
            </Link>
            <Link to="/player-registration"
      ...
      {/* ── VIDEO BACKGROUND (pinned while the next section scrolls over it) ── */}
      <div className="relative" style={{ height: "180vh" }}>
        <div className="sticky top-0 h-screen w-full overflow-hidden -z-10">
          <video
            className="absolute inset-0 w-full h-full object-cover"
            autoPlay muted loop playsInline
            poster="https://images.unsplash.com/photo-1531415077968-de08abad10ab?auto=format&fit=crop&w=1920&q=80"
          >
            <source src="https://assets.mixkit.co/videos/9585/9585-720.mp4" type="video/mp4" />
          </video>
          <div className="absolute inset-0 bg-gradient-to-b from-black/60 via-black/40 to-black/70" />
          <div className="relative h-full flex flex-col items-center justify-center text-center px-4">
            <p className="text-2xs font-semibold uppercase tracking-widest text-gold-400 mb-3">Every Bid. Every Roar.</p>
            <h2 className="font-editorial text-3xl sm:text-5xl font-bold text-white max-w-2xl leading-tight">
              Feel the Stadium Energy, Live
            </h2>
          </div>
        </div>
      </div>
      <div className="relative z-10 -mt-[100vh] bg-[#0a0d0a] rounded-t-[2.5rem] shadow-[0_-20px_60px_rgba(0,0,0,0.5)]">
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-8 py-10 border-b border-white/[0.06]">
          <div className="grid sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {[
              ...
              ["fa-solid fa-user-group", "Players Registered", `${t.registeredPlayers ?? 0}`],
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-8 py-16">
        <div className="grid md:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div className="rounded-2xl overflow-hidden border border-white/[0.08]">
            <img
              src="https://images.unsplash.com/photo-1595435742656-5272d0b3fa82?auto=format&fit=crop&w=1000&q=80"
      ...
      {tournaments.length > 1 && (
        <section className="max-w-7xl mx-auto px-4 sm:px-8 py-16 border-t border-white/[0.06]">
          <h2 className="font-editorial text-3xl sm:text-4xl font-bold text-white mb-2 leading-tight">
            Tournaments Open for Registration
          </h2>
          <p className="text-ink-400 max-w-lg mb-10">See how many players have already signed up for each tournament.</p>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {tournaments.map(tt => (
              <div key={tt._id} className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-5 hover:border-gold-500/30 transition">
                <div className="flex items-center gap-3 mb-4">
                  <div className="h-11 w-11 rounded-xl bg-white/[0.06] border border-white/[0.08] overflow-hidden flex items-center justify-center shrink-0">
                    {tt.logo ? <img src={tt.logo} className="h-full w-full object-cover" alt={tt.name} /> : <i className="fa-solid fa-trophy text-gold-400" />}
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="font-bold text-white truncate">{tt.name}</p>
                    <p className="text-2xs text-ink-500 truncate">{tt.venue || "Venue TBA"}</p>
                  </div>
                  {tt.registrationOpen
                    ? <span className="text-2xs font-semibold uppercase tracking-wide text-jade-400 flex items-center gap-1.5 shrink-0"><span className="w-1.5 h-1.5 rounded-full bg-jade-400 animate-pulse" />Open</span>
                    : <span className="text-2xs font-semibold uppercase tracking-wide text-ink-500 shrink-0">Closed</span>}
                </div>
                <div className="flex items-center justify-between rounded-lg bg-black/20 border border-white/[0.06] px-4 py-3">
                  <span className="text-sm text-ink-300">Players Registered</span>
                  <span className="font-mono text-xl font-bold text-gold-400">{tt.registeredPlayers ?? 0}</span>
                </div>
                {tt.registrationOpen && (
                  <Link to="/player-registration"
      ...
      <footer className="border-t border-white/[0.06] bg-[#0a0d0a]">
        <div className="max-w-7xl mx-auto px-4 sm:px-8 py-10 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <span className="font-editorial text-base font-bold text-gold-400">BidArena Auctions</span>
            <span className="text-2xs text-ink-500">© {new Date().getFullYear()} All rights reserved.</span>
          </div>
